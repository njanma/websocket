...
class TableRepository(connector: DbConnector) {
  ...
  def save(table: Table): ConnectionIO[Table] = insertOrUpdate(SQL.insert(table))

  def update(table: Table): ConnectionIO[Table] = insertOrUpdate(SQL.update(table))
  ...
  def update(tables: List[Table]): Stream[doobie.ConnectionIO, Table] =
    Update[TableUpd](
      ...
      .updateManyWithGeneratedKeys[Table](
        ...
        "participants",
        "ordering"
      )(tables.map(table => (table.name, table.participants, table.ordering.getOrElse(0), table.id.get): TableUpd))
...
object TableRepository {
  ...
  object SQL {
    def update(table: Table): Fragment = table match {
      case Table(Some(id), name, participants, maybeOrd) =>
        sql"""update "table"
            set name=$name, participants=$participants""" ++ maybeOrd.map(ord => fr", ordering=$ord").getOrElse(fr"") ++ fr"where id = $id"
    }

    def insert(table: Table): Fragment = table match {
      case Table(_, name, participants, maybeOrd) =>
        sql"""insert into "table"(name, participants""" ++ maybeOrd.map(_ => fr" ,ordering)").getOrElse(fr")") ++ fr" values($name, $participants" ++ maybeOrd.map(ord => fr", $ord)").getOrElse(fr")")
    }
  }
